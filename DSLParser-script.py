...
def transform_idiom_description_to_dictionary(idiom_description_list : list):
    ...
    for line in idiom_description_list:
        if line.startswith('\t'):
            if line.strip().startswith('[m1]'):
                ...
                if 'meaning' in res:
                    if res['meaning'].startswith('— '):
                        semantic_element['source'] = res['meaning'].strip()
                    else:
                        semantic_element['meaning'] = res['meaning'].strip()
                if 'role' in res:
            ...
            current_element['phrase'].append(line.strip())
    current_element['semantics'].append(semantic_element)
    return current_element

print(os.getcwd())
dsl_fname = os.path.join('DSLDictionary', 'rus-rus_phr_fedor.dsl')
dsl_dictionary = open(dsl_fname, 'r', encoding='utf-8')
...
with open('result/idioms.json', 'w', encoding='utf8') as fp:
